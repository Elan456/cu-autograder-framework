...
class JSONTestResult(result.TestResult):
    ...
    def __init__(
        self, stream, descriptions, verbosity, results, leaderboard, failure_prefix
    ):
    ...
    def getLeaderboardData(self, test):
        column_name = getattr(
            getattr(test, test._testMethodName), "__leaderboard_column__", None
        )
        sort_order = getattr(
            getattr(test, test._testMethodName), "__leaderboard_sort_order__", None
        )
        value = getattr(
            getattr(test, test._testMethodName), "__leaderboard_value__", None
        )
